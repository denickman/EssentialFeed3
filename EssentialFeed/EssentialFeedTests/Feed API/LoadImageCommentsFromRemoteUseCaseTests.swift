...
final class LoadImageCommentsFromRemoteUseCaseTests: XCTestCase {
    ...
    func test_load_deliversErrorOn2xxHTTPResponseWithInvalidData() {
        let (sut, client) = makeSUT()
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: failure(.invalidData), when: {
                ...
                client.complete(with: code, data: invalidData, at: index)
            })
        }
    }
    
    func test_load_deliversNoItemsOn2xxHTTPResponseWithEmptyData() {
        let (sut, client) = makeSUT()
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: .success([])) {
                ...
                client.complete(with: code, data: emtpyData, at: index)
            }
        }
    }
    
    func test_load_devliersItemsOn2xxHTTPResponseWithJSONItems() {
        let (sut, client) = makeSUT()
        
        let item1 = makeItem(
            id: UUID(),
            message: "a message",
            createdAt: (Date.init(timeIntervalSince1970: 1598627222), "2020-08-28T15:07:02+00:00"),
            username: "a username"
        )
        ...
        let item2 = makeItem(
            ...
            createdAt: (Date.init(timeIntervalSince1970: 1577881882), "2020-01-01T12:31:22+00:00"),
